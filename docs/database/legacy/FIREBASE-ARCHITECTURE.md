# Legacy Firebase architecture — historical reference only

Superseded by `../ARCHITECTURE.md` and the Supabase migrations. The following describes the previous Firebase implementation, not the active provider or current deployment status. Retained for migration review and rollback planning.
